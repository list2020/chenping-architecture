...
ace.handle_side_menu = function(a) {
	...
	a(".nav-list").on(ace.click_event, function(h) {
		...
		//FIXME 设置延迟用于确定面板不超出大小
		setTimeout(function(){
			a(f).slideToggle(200).parent().toggleClass("open");
			setTimeout(function(){
				var li_num = $(".nav-list").children("li").length;
				var child_li_num = $(".nav-list").find(".open").find("ul").children("li").length;
				if(child_li_num>0){
					var innerHeight = $("#sidebar").height()-(li_num*$(".dropdown-toggle").height()+12)-45;
					$(".nav-list").find(".open").find("ul").eq(0).css({"height":innerHeight+"px","overflow-y":"auto"});
				}
			},220);
